...
```
Для проверки смотрим маршрут 10.4.2.0/24 который должен прийти из ЦОД2, ищем его на BLeaf1. Видим remote маршрут из ЦОД2 и созданный маршрут, который отдается в фабрику, показывая, что BLeaf1 знает о этой  сети (10.254.253.98 from - (0.0.0.0)) 
```python
BLeaf1#sh bgp evpn route-type ip-prefix 10.4.2.0/24
BGP routing table information for VRF default
Router identifier 10.255.252.98, local AS number 65098
BGP routing table entry for ip-prefix 10.4.2.0/24, Route Distinguisher: 10.254.252.1:1
 Paths: 1 available
  65198 65100 65101
    10.254.253.98 from - (0.0.0.0)
      Origin IGP, metric -, localpref 100, weight 0, tag 0, valid, external, best
      Extended Community: Route-Target-AS:1:100666 TunnelEncap:tunnelTypeVxlan EvpnRouterMac:50:00:00:d5:e2:ad
      VNI: 100666
...
В каждом из ЦОД реализован выход в Интернет через двух провайдеров. ЦОД1 - ISP1 основной, ISP2 резервный. ЦОД2 - ISP3 - основной, ISP4 - резервный. В каждом ЦОД есть Nat устройство (считаем что кластер, просто в лабе тяжело сделать уже). Входящие интерфейсы смотрят в vrf Customer1, исходящие отдельные влан в сторону Интернет провайдеров через BGP_Border`ы. Внешняя сеть 82.1.1.0/24 в ЦОД1 (закрывает 10.4.0.0/24 и 10.4.1.0/24 сети). Внешняя сеть 83.1.1.0/24 в ЦОД2 (закрывает 10.4.2.0/24). Но в случае падения провайдеров в ЦОД1 трафик в Интернет идет через ЦОД2. В сторону провайдеров анонсируем только внешние сети, в сторону фабрик дефолты. Сами дефолты зарождаются только на провайдерах (neighbor default-originate), это  сделано для того, чтобы при падении провайдеров дефолт пропадал.    
